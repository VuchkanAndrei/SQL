...
--Функция
CREATE FUNCTION SalaryTable(@Month INT, @Date DATE, @Salary MONEY, @PremiumStavka MONEY)
RETURNS @resultTable table (Month INT, Date DATE, Salary MONEY, Premium MONEY)
AS
BEGIN
DECLARE @Premium MONEY = 0
DECLARE @CountMonth INT = @Month
WHILE @Month>0
BEGIN
INSERT INTO @resultTable VALUES (@CountMonth-@Month+1, @Date, ROUND(@Salary,2), ROUND(@Premium,2))
SET @Month=@Month-1
set @Date=DATEADD(MONTH,1,@Date)
set @Premium=@Salary*@PremiumStavka-@Salary
set @Salary=@Salary+@Premium
END
RETURN
END

DROP FUNCTION SalaryTable

SELECT * FROM dbo.SalaryTable(30,'01.01.2020',1000,1.02)

--Процедура
CREATE PROCEDURE SalaryTableProcedure @Month INT, @Date DATE, @Salary MONEY, @PremiumStavka MONEY
AS
BEGIN
DECLARE @resultTable table (Month INT, Date DATE, Salary MONEY, Premium MONEY)
DECLARE @Premium MONEY = 0
DECLARE @CountMonth INT = @Month
WHILE @Month>0
BEGIN
...
DROP PROCEDURE SalaryTableProcedure

EXEC SalaryTableProcedure 30,'01.01.2020',1000,1.02
